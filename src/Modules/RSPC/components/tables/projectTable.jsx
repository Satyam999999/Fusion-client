...
function ProjectTable({ projectsData, activeRole }) {
  ...
  const rows = sortData(projectsData || []).map((row, i) => (
    <Table.Tr key={row.id || i}>
      <Table.Td className={classes["row-content"]}>
        <Badge color={badgeColor[row.status] || "gray"} size="lg" style={{ minWidth: "120px", color: "#3f3f3f" }}>
          {row.status}
        </Badge>
      </Table.Td>
      <Table.Td className={classes["row-content"]} style={{ maxWidth: 260, textAlign: "left" }}>{row.title}</Table.Td>
      <Table.Td className={classes["row-content"]}>{row.project_number || "-"}</Table.Td>
      <Table.Td className={classes["row-content"]}>{row.pi_name || "-"}</Table.Td>
      <Table.Td className={classes["row-content"]}>{row.funding_agency_name || row.funding_agency || "-"}</Table.Td>
      <Table.Td className={classes["row-content"]}>
        {row.sanctioned_amount ? `INR ${Number(row.sanctioned_amount).toLocaleString("en-IN")}` : "-"}
      </Table.Td>
      <Table.Td className={classes["row-content"]}>{row.start_date ? new Date(row.start_date).toLocaleDateString() : "-"}</Table.Td>
      <Table.Td className={classes["row-content"]}>{row.original_end_date ? new Date(row.original_end_date).toLocaleDateString() : "-"}</Table.Td>
      <Table.Td className={classes["row-content"]}>
        <Button onClick={() => handleAction(row)} variant="outline" color="#15ABFF" size="xs" style={{ borderRadius: "8px" }}>
  ...
  return (
    <>
      <ScrollArea h={420} onScrollPositionChange={({ y }) => setScrolled(y !== 0)}>
        <Table highlightOnHover>
          <Table.Thead className={cx(classes.header, { [classes.scrolled]: scrolled })}>
            <Table.Tr>
              <SortTh label="Status" col="status" />
              <SortTh label="Title" col="title" />
              <SortTh label="Project No." col="project_number" />
              <SortTh label="PI" col="pi_name" />
              <SortTh label="Funding Agency" col="funding_agency_name" />
              <SortTh label="Amount" col="sanctioned_amount" />
              <SortTh label="Start Date" col="start_date" />
              <SortTh label="End Date" col="original_end_date" />
          ...
          <Table.Tbody>
            {(projectsData || []).length === 0
              ? (
                <Table.Tr>
                  <Table.Td colSpan={9}>
                    <Text ta="center" c="dimmed" py="md">No projects found</Text>
                  </Table.Td>
                </Table.Tr>
                )
              : rows}
      ...
      <DetailViewModal
        ...
        fields={[
          ...
          { label: "Funding Agency", key: "funding_agency_name" },
          {
            ...
            format: (value) => (value ? `INR ${Number(value).toLocaleString("en-IN")}` : "-"),
          },
          {
            label: "Start Date",
            key: "start_date",
