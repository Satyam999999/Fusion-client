...
import { fetchPatentsRoute } from "../../routes/RSPCRoutes/index";

const PATENT_TYPES = [
  {value:"NATIONAL",label:"National"},{value:"INTERNATIONAL",label:"International"},
  {value:"PCT",label:"PCT"},{value:"DESIGN",label:"Design"},
];
...
function Patents() {
  ...
  const [form, setForm] = useState({
    title:"", patent_type:"NATIONAL", application_number:"",
    filing_date:"", country:"India", abstract:"", external_inventors:"",
  });
  ...
  const handleSubmit = async () => {
    if (!form.title) {
      notifications.show({title:"Validation",message:"Title is required",color:"red"});
      return;
    }
    try {
      await axios.post(fetchPatentsRoute, form);
      notifications.show({title:"Filed",message:"Patent application filed",color:"green"});
      loadPatents();
      setActiveTab("0");
    } catch(e) {
      notifications.show({title:"Error",message:"Failed to file patent",color:"red"});
    }
  };
  ...
  const tabItems = [
    {
      ...
      component: loading ? <Center py="xl"><Loader size="lg"/></Center> :
        <PatentsTable patents={patents} onRefresh={loadPatents}
    ...
    },
    {
      ...
      component: (
        <Paper className={formClasses.formContainer}>
          <Title order={4} className={formClasses.formTitle}>File New Patent Application</Title>
          <Grid gutter="md">
            <Grid.Col span={12}>
              <TextInput label="Patent Title" required value={form.title}
                onChange={e=>setForm(f=>({...f,title:e.target.value}))}/>
            </Grid.Col>
            <Grid.Col span={6}>
              <Select label="Patent Type" data={PATENT_TYPES} value={form.patent_type}
                onChange={v=>setForm(f=>({...f,patent_type:v}))}/>
            </Grid.Col>
            <Grid.Col span={6}>
              <TextInput label="Application Number" value={form.application_number}
                onChange={e=>setForm(f=>({...f,application_number:e.target.value}))}/>
            </Grid.Col>
            <Grid.Col span={6}>
              <TextInput label="Filing Date" type="date" value={form.filing_date}
                onChange={e=>setForm(f=>({...f,filing_date:e.target.value}))}/>
            </Grid.Col>
            <Grid.Col span={6}>
              <TextInput label="Country" value={form.country}
                onChange={e=>setForm(f=>({...f,country:e.target.value}))}/>
            </Grid.Col>
            <Grid.Col span={12}>
              <TextInput label="Inventors" value={form.external_inventors}
                onChange={e=>setForm(f=>({...f,external_inventors:e.target.value}))}
          ...
          <div className={formClasses.submitButtonContainer}>
            <Button color="#15ABFF" style={{borderRadius:8}} onClick={handleSubmit}
              leftSection={<CheckCircle size={18}/>}>File Patent</Button>
          </div>
        </Paper>
      ),
    },
    ...
    <>
      ...
      <DetailViewModal opened={viewOpen} onClose={()=>setViewOpen(false)}
        ...
        fields={[
          ...
          {label:"Filing Date",key:"filing_date",format:v=>v?new Date(v).toLocaleDateString():"—"},
          {label:"Grant Date",key:"grant_date",format:v=>v?new Date(v).toLocaleDateString():"—"},
